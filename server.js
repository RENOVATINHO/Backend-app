...
//mudar essa tabela pra oque o hete pediu
...
app.post('/register', async (req, res) => {
    const { username, password, dogName} = req.body;  // Role padrão para 'user'
    try {
...
const validateSensorData = (req, res, next) => {
    ...
    for (const dados of dadosArray) {
        const { sensor_id, temperatura, luminosidade, ocupação } = dados;
        ...
        if (!luminosidade || typeof luminosidade !== 'number') {
            return res.status(400).json({ message: 'luminosidade inválida ou ausente.' });
        }
        if (!ocupação || typeof ocupação !== 'number') {
            return res.status(400).json({ message: 'ocupação inválida ou ausente.' });
        }
    }
...
app.post('/dados-sensores', authenticateJWT, validateSensorData,(req, res) => {
    ...
    const insertPromises = dadosArray.map(dados => {
        return new Promise((resolve, reject) => {
            db.run(`INSERT INTO dados_sensores (sensor_id, temperatura, luminosidade, ocupação) VALUES (?, ?, ?, ?)`,
                [dados.sensor_id, dados.temperatura, dados.luminosidade, dados.ocupação],
                (err) => {
                    if (err) {
